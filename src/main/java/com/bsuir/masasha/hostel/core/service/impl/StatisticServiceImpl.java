...
public class StatisticServiceImpl implements StatisticService {
    ...
    @Override
    public List<StatisticLineDTO> calculateProfitLines(Integer year) {
        List<StatisticLineDTO> lines = new ArrayList<>();
        String yearString = year + "-01-01";
        ...
        List<Double> dots = new ArrayList<>();
        for (int i = 1; i <= 12; i++) {
            double count = jdbcTemplate.queryForObject(
                    "SELECT sum(r.cost) FROM reservation r WHERE " +
                            "start_date between ? + interval " + i + " month and ? + interval " + (i + 1) + " month;",
